...
function goBack() {
  if      (role.value === "doctor")  router.push("/doctor");
  else if (role.value === "patient") router.push("/patient");
  else if (role.value === "admin")   router.push("/admin");
  else                               router.push("/");
}
...
<template>
  <div class="page">
    ...
    <h2 class="header">Patient History</h2>
    <p v-if="role === 'admin'"><strong>Admin Name:</strong> {{ viewerName }}</p>
    <p><strong>Patient Name:</strong> {{ patient.name }}</p>
...
const roleRoutes = { doctor: "/doctor", patient: "/patient", admin: "/admin" };
const goBack = () => router.push(roleRoutes[role.value] || "/");

async function fetchHistory() {
  const id = route.params.id;
  const urls = {
    doctor:  `${BASE}/api/doctor/patient/${id}/history`,
    admin:   `${BASE}/api/admin/patient/${id}/history`,
    patient: `${BASE}/api/patient/history`,
  };

  try {
    const res  = await fetch(urls[role.value], { headers: authHeaders() });
    const data = await res.json();
    patient.value = data.patient || { name: "-" };
    history.value = data.history || [];
  } catch (err) {
...
async function exportCSV() {
  ...
  exportMsg.value = "";
  try {
    const res = await fetch(`${BASE}/api/export-csv`, { method: "POST", headers: authHeaders() });
    exportMsg.value = res.ok ? "Export started! You will receive an email shortly." : "Export failed. Please try again.";
  } catch {
    exportMsg.value = "Something went wrong.";
  } finally {
    exporting.value = false;
...
.back-btn { float: right; background-color: rgb(2, 33, 33); color: white; border: 1px solid; padding: 5px 12px; border-radius: 6px; text-decoration: none; cursor: pointer; }
...
.export-btn { background-color: rgb(2, 33, 33); color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-size: 14px; }
.export-btn:disabled { opacity: 0.6; cursor: not-allowed; }
.export-msg { margin-left: 12px; font-size: 14px; color: #2a6b2a; }
</style> -->
<!-- 
<template>
  <div class="page">
