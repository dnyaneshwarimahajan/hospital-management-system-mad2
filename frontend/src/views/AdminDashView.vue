  ...
  <div class="container">
    ...
    <div class="summary">
      ...
      <div class="create_doc card">
        <button class="btn btn_create" @click="$router.push('/create-doctor')">
          Create doctor
        </button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div v-if="doctors.length">
        <div class="user-row" v-for="d in filteredDoctors" :key="d.id">
          ...
          <div class="btn_edit">
            <button class="btn edit" @click="editDoctor(d.id)">Edit</button>

            <button class="btn delete" @click="deleteDoctor(d.id)">
              Delete
            </button>

            <button
              class="btn"
              :class="d.blacklisted ? 'unblock' : 'black'"
      ...
      <div v-if="patients.length">
        <div class="user-row" v-for="p in filteredPatients" :key="p.id">
          ...
          <div class="btn_edit">
            ...
            <button class="btn delete" @click="deletePatient(p.id)">
              Delete
            </button>

            <button
              class="btn"
              :class="p.blacklisted ? 'unblock' : 'black'"
    ...
    <div class="card" style="margin-top:14px;">
      ...
      <button class="btn btn_create" @click="$router.push('/past-appointments')">
...
const filteredDoctors = computed(() =>
  doctors.value.filter(d =>
    ...
    (d.department || "").toLowerCase().includes(searchQuery.value.toLowerCase())
  )
);
...
// Navigation methods
const editDoctor = (id) => router.push(`/edit-doctor/${id}`);
const editPatient = (id) => router.push(`/edit-patient/${id}`);
const viewHistory = (id) => router.push(`/patient-history/${id}`);

// Delete and toggle methods
...
<style scoped>
/* ✅ Your original CSS kept exactly */
...
.user-row { display: flex; justify-content: space-between; padding: 12px 10px; border-bottom: 5px solid #b4f0f0ab; }
.user-row:last-child { border-bottom: none; }
.user-left { font-weight: 600; font-size: 1.05rem; font-style: italic; color: #054145; }
.user-meta { color: #3559a1; font-size: 0.9rem; margin-top: 3px; }
.btn { padding: 8px 12px; border-radius: 8px; border: none; cursor: pointer; font-weight: 600; font-size: 0.95rem; }
.edit{ background-color: #1ff3868d; color: #111827; }
.delete{ background-color: #e66969b8; color:#2f0303; }
.black{ background-color: #05132f9e; }
.unblock{ background-color: #066f77c3; }
.btn_create{ background-color: #052c2fe2; color: white; }
.muted { color: #6b7280; font-size: 0.9rem; }
.summary{ display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px; }
.one{ flex:1;min-width:220px }
.doc{ flex:1;min-width:120px; }
.rest{ flex:1;min-width:120px }
.create_doc{ flex:1;min-width:320px }
.badge{ color:#2f0303;font-weight:600;margin-left:8px }
</style>
